...
export default async function CustomerExhibitionsPage() {
  let list: Exhibition[] = [];
  let error: string | null = null;
  try {
    const res = await axios.get(`${API_BASE_URL}/customer/exhibitions`, { headers: {} });
    list = res.data || [];
  } catch (err: any) {
      ...
      <main className="p-8 max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {list.map((ex) => {
            ...
            return (
              <Link key={ex.exhibition_id} href={`/customer/exhibitions/${ex.exhibition_id}`} className="block">
